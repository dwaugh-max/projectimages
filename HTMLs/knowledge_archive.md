# Classroom Sim Architect: Knowledge Archive (v65.8 - synced with codebase)
...
<body>
    ...
    <script>
        ...
        function applyTheme() {
            ...
            document.getElementById('f-left').innerText = `${m.title || currentMission}`;
...
Provide a concise assessment (150 words). Format with markdown.`;
                        ...
                        // SILENT FAILSAFE: Show static content instead of error
                        // Also set aiFeedback to static so we don't try again on refresh
                        user.state.aiFeedback = staticContent; // optional: save the static content as the "feedback"
                        save();
                        renderDebrief();
                    }
                }
        ...
        function tab(el, tName) {
            ...
            if (isYouTube) {
                ...
                if (vidId) {
                    h = `<iframe src="https://www.youtube-nocookie.com/embed/${vidId}?rel=0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen style="width:100%;height:100%;border:none;" referrerpolicy="no-referrer"></iframe>`;
                } else {
        ...
        async function showDebugInfo() {
            ...
            document.getElementById('dbg-engine').innerText = "v63.1 (Failsafe + Hardening)";
    ...
    <div id="debug-modal" class="modal"
        ...
        <div style="background:#111; border:1px solid var(--accent); padding:30px; width:400px; text-align:center;">
            <h2 style="color:var(--accent); margin-top:0;">COMPONENT STATUS</h2>
            <div
                style="text-align:left; font-family:var(--font-h); font-size:0.9rem; line-height:2; margin:20px 0; color:#ddd;">
                <div><strong>ENGINE (Index):</strong> <span id="dbg-engine"
                        style="float:right; color:var(--accent);">...</span></div>
                <div><strong>MISSION DATA (Blob):</strong> <span id="dbg-blob"
                        style="float:right; color:var(--accent);">...</span></div>
